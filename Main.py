...
ALPHABET_PIN = 25
WORD_PIN = 27
GPIO.setup(ALPHABET_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(WORD_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
class App:
    def __init__(self, master):
        ...
        GPIO.add_event_detect(ALPHABET_PIN, GPIO.FALLING, callback=self.run_testing_alphabet, bouncetime=300)
        GPIO.add_event_detect(WORD_PIN, GPIO.FALLING, callback=self.run_testing_word, bouncetime=300)

    def run_testing_alphabet(self, channel=None):
        self.run_script('TestingAlphabetsTflite.py')

    def run_testing_word(self, channel=None):
        self.run_script('TestingWordTflite.py')
...
if __name__ == "__main__":
    ...
    app = App(root)
    root.mainloop()
    GPIO.cleanup()  # Clean up GPIO pins on exitimport customtkinter as tk

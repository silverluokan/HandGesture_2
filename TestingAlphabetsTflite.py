...
def download_model_from_storage(storage_path, local_path):
    blob = bucket.blob(storage_path)
    blob.download_to_filename(local_path)
    print(f"Downloaded {storage_path} to {local_path}")

# Download models
download_model_from_storage('models/alphabets_model.keras', 'alphabets_model.keras')
download_model_from_storage('models/alphabets_model.tflite', 'alphabets_model.tflite')
...
# LANG_UP_PIN = 4
# LANG_DOWN_PIN = 5
# SPACE_PIN = 6  # SPACE button pin
# CLOSE_PIN = 25  # Close button pin
#
...
# GPIO.setup(SPACE_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # Setup for SPACE button
# GPIO.setup(CLOSE_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # Setup for CLOSE button
#
# class CustomFrameApp:
#     def __init__(self, master):
#         self.master = master
...
#         GPIO.add_event_detect(SPACE_PIN, GPIO.FALLING, callback=self.add_space, bouncetime=300)  # Event for SPACE button
#         GPIO.add_event_detect(CLOSE_PIN, GPIO.FALLING, callback=self.close_window, bouncetime=300)  # Event for CLOSE button
#
#         self.show_camera_feed()
#
#     def setup_ui(self):
...
#         self.language_combobox.pack(side=tk.LEFT, padx=10, pady=20)
#
#         self.language_var.trace("w", self.translate_sentence)  # Trace the language_var to call translate_sentence on change
#
#     def setup_camera(self):
#         self.video_capture = cv2.VideoCapture(0)
#         self.camera_label = tk.CTkLabel(self.MainCamera)

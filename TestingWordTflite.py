...
# Download models
download_model_from_storage('models/words_model.keras', 'words_model.keras')
download_model_from_storage('models/words_model.tflite', 'words_model.tflite')
...
class CustomFrameApp:
    def __init__(self, master):
        ...
        self.actions = firebase.get('/words_model', None)
        print(self.actions)
        self.sequence = []

        # Load the TFLite model and allocate tensors
        self.interpreter = tf.lite.Interpreter(model_path='words_model.tflite')
...
# GPIO.setup(CLOSE_APP_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
#
# class CustomFrameApp:
#     def __init__(self, master):
#         self.master = master
...
#         GPIO.add_event_detect(CLOSE_APP_PIN, GPIO.FALLING, callback=self.close_app, bouncetime=300)
#
#         self.show_camera_feed()
#
#     def setup_ui(self):
...
#     app = CustomFrameApp(root)
#     root.mainloop()
#     GPIO.cleanup()  # Clean up GPIO pins on ex
#
#
